...
public class TerminalController {
	...
	@objc private func updateTimerFired() {
		terminalQueue.async {
			...
			if !self.pendingLineUpdates.isEmpty {
				linesToUpdate.formUnion(self.pendingLineUpdates)
				self.pendingLineUpdates.removeAll()
			}

			// 限制每次更新的行数，避免一次性更新过多导致主线程阻塞
			let linesToUpdateArray = Array(linesToUpdate)
			let immediateUpdates: [Int]
			let pendingUpdates: [Int]
			
			if linesToUpdateArray.count > maxLinesPerUpdate {
				// 如果更新行数过多，分批处理
				let splitIndex = maxLinesPerUpdate
				immediateUpdates = Array(linesToUpdateArray[0..<splitIndex])
				pendingUpdates = Array(linesToUpdateArray[splitIndex...])
				self.pendingLineUpdates = Set(pendingUpdates)
			} else {
				immediateUpdates = linesToUpdateArray
				pendingUpdates = []
			}

			// 更新立即需要显示的行
			for i in immediateUpdates {
			...
			DispatchQueue.main.async {
				...
				// 如果有待处理的更新，延迟处理以避免阻塞
				if !self.pendingLineUpdates.isEmpty && !self.isProcessingPendingUpdates {
	...
	private func processPendingUpdates() {
		guard !pendingLineUpdates.isEmpty && !isProcessingPendingUpdates else {
			return
		}
		
		isProcessingPendingUpdates = true
		
		terminalQueue.async {
			// 每次处理一批更新
			let batchSize = self.maxLinesPerUpdate
			...
			DispatchQueue.main.async {
				self.delegate?.refresh(lines: &self.lines)
				self.isProcessingPendingUpdates = false
				
				// 如果还有待处理的更新，继续处理
				if !self.pendingLineUpdates.isEmpty {
					// 延迟一小段时间，避免连续更新导致主线程阻塞
	...
	public func clearTerminal() {
		...
		pendingLineUpdates.removeAll()
		isProcessingPendingUpdates = false
